...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.gsap && window.ScrollTrigger) {
    ...
    const productShowcase = document.querySelector(
  ...
  function updateProductCategory(categoryKey) {
    ...
    productTabs.forEach((tab) => {
      ...
      const icon = tab.querySelector("svg, i");
      if (isSelected) {
        tab.className =
          "product-tab-btn min-h-[46px] px-5 py-2.5 rounded-xl font-sans font-semibold text-sm sm:text-base transition-all duration-200 flex items-center justify-center gap-2 whitespace-nowrap bg-glanz-red text-white border border-glanz-red shadow-industrial-sm cursor-pointer shrink-0";
        if (icon) {
          icon.classList.remove("text-glanz-red");
          icon.classList.add("text-white");
        }
      } else {
        tab.className =
          "product-tab-btn min-h-[46px] px-5 py-2.5 rounded-xl font-sans font-semibold text-sm sm:text-base transition-all duration-200 flex items-center justify-center gap-2 whitespace-nowrap bg-white text-glanz-charcoal-900 border border-glanz-border hover:bg-glanz-red-50 hover:border-glanz-red cursor-pointer shrink-0";
        if (icon) {
          icon.classList.remove("text-white");
          icon.classList.add("text-glanz-red");
        }
      }
    });
    ...
    if (window.gsap) {
      ...
      if (displayBadgeBottomText)
        displayBadgeBottomText.textContent = data.bottomBadge;
      if (displayEyebrow) displayEyebrow.textContent = data.eyebrow;
      if (displayTitle) displayTitle.textContent = data.title;
      if (displayDesc) displayDesc.textContent = data.desc;
      if (displayFeatures) {
